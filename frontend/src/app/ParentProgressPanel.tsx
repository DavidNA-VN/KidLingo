    ...
    <section className="rounded-xl border border-[#dfe6ef] bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[#667085]">Theo dõi bài đã làm, độ chính xác, reward và feedback giáo viên.</p>
        </div>
        {dashboard && (
          <span className="rounded-full bg-[#edf4ff] px-3 py-1.5 text-xs font-bold text-[#155dcc]">
            {dashboard.active_child_count} hồ sơ active
          </span>
        )}
      </div>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-lg bg-[#f8fafc] p-4">
              ...
              <div className="mt-1 text-xs font-semibold text-[#667085]">Tổng reward</div>
            </div>
            <div className="rounded-lg bg-[#f8fafc] p-4">
              <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[#667085]">
                <Coins size={14} />
          ...
          <div className="mt-5">
            <div className="mb-3 flex items-center justify-between">
              <h4 className="font-bold">Lịch sử gần đây</h4>
            ...
            <div className="space-y-3">
                ...
                <article key={submission.id} className="rounded-lg border border-[#e4eaf2] bg-[#fbfcfe] p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-[#344054]">
                    {submission.submission_type === "PDF_ANSWER" ? (
                      ...
                      <>
                        <span className="rounded-full bg-white px-2.5 py-1">AI: {((submission.confidence ?? 0) * 100).toFixed(1)}%</span>
                        <span className="rounded-full bg-white px-2.5 py-1">
                          {submission.speech_passed ? "Đọc đạt" : "Cần luyện đọc"}
                        </span>
                        <span className="rounded-full bg-white px-2.5 py-1">
                          +{submission.stars_earned} sao · +{submission.coins_earned} xu
                        </span>
                      </>
                    )}
                  </div>
              ...
              {!progress.recent_submissions.length && (
                <div className="rounded-lg bg-[#f8fafc] p-4 text-sm text-[#667085]">
                  Chưa có bài nộp. Khi bé hoàn thành doodle game, lịch sử sẽ xuất hiện ở đây.
                </div>
              )}
            </div>
          </div>
